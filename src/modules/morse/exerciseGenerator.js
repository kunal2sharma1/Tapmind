...
function candidateWeight(character, context) {
  ...
  const review = context.reviews?.[character.symbol];
  const due = context.dueCharacterIds?.includes(character.id) ||
    (review?.dueAt && Date.parse(review.dueAt) <= (context.now ? new Date(context.now).getTime() : Date.now()));
  const overdueAgeHours = due && review?.dueAt
    ? Math.max(0, ((context.now ? new Date(context.now).getTime() : Date.now()) - Date.parse(review.dueAt)) / 3600000)
    : 0;
  const reviewBoost = due ? 5 + Math.min(overdueAgeHours, 24) * 0.25 : 0;

  return (1 + reviewBoost + weak * 3 + unseen * 1.5) * recentPenalty;
}
...
export function rankCharacters(candidates, context = {}) {
  const baseRanked = context.characterMastery
  ...
  return baseRanked
    .map((entry) => ({
      ...entry,
      weight: entry.weight + candidateWeight(entry.character, context) * 0.85,
    }))
    .sort((a, b) => {
...
export function generateExercise({
  ...
  return Object.freeze({
    ...
    generator: Object.freeze({
      ...
      scheduledReview: Boolean(context.reviews?.[selected.symbol]),
    }),
